...
**Location.** `config.py:14` `LOG_ENCRYPTION_PASSWORD = "DFX"` and `config.py:17`
`ADMIN_REPORT_PASSWORD = "DFX"`. The Fernet key for the "encrypted" system logs
is derived (PBKDF2, fixed salt `b"DFX-ATE-SALT-v1"`) from that constant
...
**Location.** `DatabaseManager._ensure_initial_admin` seeds Admin `lior` with
password `Aa123456` ([database_manager.py:116](src/logic/database_manager.py#L116)).
...
aligning to "Technician" / `Aa123456` semantics in a follow-up.
